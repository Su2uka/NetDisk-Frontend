...
FluWindow {
    ...
    function playMedia() {
        ...
        mediaPlayer.play()
        var name = playlistInfo.getDisplayName()
        root.title = name || qsTr("Multimedia Player")
    }
    ...
    function openFile(path, displayName) {
        ...
        ++currentFile
        playlistInfo.addFile(currentFile, path, name)
        mediaPlayer.source = path
        mediaPlayer.play()
        root.title = name || qsTr("Multimedia Player")
    }

    // 关闭窗口时只暂停播放并隐藏，不销毁（保持播放列表）
    ...
    VideoOutput {
        ...
        onFullScreenChanged: {
            // 全屏时隐藏 FluWindow 工具栏
            root.fitsAppBarWindows = fullScreen
            root.appBar.visible = !fullScreen
        }
    ...
    Image {
        ...
        width: Math.min(videoOutput.width, videoOutput.height) * 0.45
        height: width
    ...
    Rectangle {
        ...
        anchors.top: playbackControl.top
    ...
    PlaylistInfo {
        ...
        onPlayFileRequested: (fileIndex) => {
            root.currentFile = fileIndex
            root.playMedia()
        }
    }
